...
class Item(models.Model):
    ...
    created_date = models.DateTimeField(default=datetime.now, verbose_name='创建时间', help_text="创建时间")
    completed = models.BooleanField(default=False, verbose_name='是否完成', help_text="是否完成")
    categorys = models.ForeignKey(Category, on_delete=models.CASCADE, related_name='items', help_text="大类")  #反向引用时的引用名是items
...
class Article(models.Model):
    ...
    publish_date = models.DateTimeField(default=datetime.now, verbose_name='发布日期', help_text="发布日期")
...
class UserFav(models.Model):
    ...
    class Meta:
        verbose_name = '用户收藏'
        verbose_name_plural = verbose_name
        unique_together = ("user", "articles")   #联合唯一索引
